...
def add(A, B):
    ...
    for d0 in A.D[0]:
        for d1 in A.D[1]:
            output[(d0,d1)] = getitem(A,(d0,d1))+getitem(B,(d0,d1))
    return Mat(A.D, output)
...
def scalar_mul(M, alpha):
    ...
    for d0 in M.D[0]:
        for d1 in M.D[1]:
            output[(d0,d1)] = getitem(M,(d0,d1)) * alpha
    return Mat(M.D, output)
...
def vector_matrix_mul(v, M):
    ...
    for d1 in M.D[1]:
        for d0 in v.D:
            output[d1] = output[d1] + v[d0]*M[d0,d1]
    return output
...
def matrix_vector_mul(M, v):
    ...
    for d0 in M.D[0]:
        for d1 in v.D:
            output[d0] = output[d0] + v[d1]*M[d0,d1]
    return output
...
def matrix_matrix_mul(A, B):
    ...
    for d0 in A.D[0]:
        for d1 in B.D[1]:
            for k in B.D[0]:
                output[d0,d1] = output[d0,d1] + A[d0,k]*B[k,d1]
    return output
