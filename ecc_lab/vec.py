...
def add(u,v):
    ...
    output = {}
    for d in u.D:
        output[d] = getitem(u,d)+getitem(v,d)
    return Vec(u.D, output)
...
def dot(u,v):
    ...
    output = 0
    for d in u.D:
        output = output + getitem(u,d)*getitem(v,d)
    return output
...
def scalar_mul(v, alpha):
    ...
    output = {}
    for d in v.D:
        output[d] = getitem(v,d) * alpha
    return Vec(v.D, output)

def neg(v):
    "Returns the negation of a vector"
    output = {}
    for d in v.D:
        output[d] = getitem(v,d) * -1
    return Vec(v.D, output)
